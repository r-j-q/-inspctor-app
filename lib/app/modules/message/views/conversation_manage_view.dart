...
class _ConversationManageViewState extends State<ConversationManageView> {
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(
        title: Text("会话列表", style: TextStyle(fontSize: 20)),
        leading: IconButton(
      ...
      children: <Widget>[
        ...
        Row(
          ...
          children: <Widget>[
            ...
            Text(" "),
            CustomButton(title: "获取历史消息", onPressed: demoGetHistorMessage),
            Text(" "),
            CustomButton(
                title: "刷新会话列表",
                onPressed: () {
  ...
  Widget _buildRow(JMConversationInfo object, int index) {
    ...
    return Container(
      ...
      child: ListTile(
        ...
        //设置内容边距，默认是 16
        title: Text(title),
        subtitle: Text(object.latestMessage.toString()),
  ...
  void demoSendTextMessage() async {
    ...
    JMTextMessage msg =
        await selectConversationInfo!.sendTextMessage(text: "send msg queen index $textIndex");
    setState(() {
      _loading = false;
