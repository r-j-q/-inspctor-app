...
import 'package:inspector/app/modules/message/views/conversation_manage_view.dart';
import 'package:inspector/app/modules/message/views/group_manage_view.dart';
...
const String kMockAppkey = "661b3ac7cffebe8fe9fbe4cd"; //'你自己应用的 AppKey';
const String kMockUserName = 'user20479';
const String kMockPassword = '123456';
const String kCommonPassword = '123456';
...
final JMGroup kMockGroup = JMGroup.fromJson({'type': JMGroupType.private, 'groupId': kMockGroupId});

const String kMockChatRoomid = '10003152';
final JMChatRoom kMockChatRoom = JMChatRoom.fromJson({'roomId': kMockChatRoomid});

MethodChannel channel = MethodChannel('jmessage_flutter');
JmessageFlutter jmessage = JmessageFlutter.private(channel, const LocalPlatform());
...
class _MyHomePageState extends State<MyHomePage> {
  ...
  void initState() {
    ...
    jmessage.applyPushAuthority(JMNotificationSettingsIOS(sound: true, alert: true, badge: true));
    addListener();
  }
  ...
  void demoLoginUserAction() async {
    ...
    String name = usernameTextEC1.text;
    await jmessage.login(username: name, password: kCommonPassword).then((onValue) {
      setState(() {
        _loading = false;
  ...
  void demoSendLocationMessage() async {
    ...
    JMLocationMessage msg = await jmessage.sendLocationMessage(
        type: type, latitude: 100.0, longitude: 200.0, scale: 1, address: "详细地址");
    setState(() {
      _loading = false;
  ...
  void demoSendVideoMessage() async {
    ...
    PickedFile? selectVideoPath = await ImagePicker()
        .getVideo(source: ImageSource.gallery, maxDuration: const Duration(seconds: 10));
  ...
  void addListener() async {
    ...
    jmessage.addReceiveApplyJoinGroupApprovalListener((JMReceiveApplyJoinGroupApprovalEvent event) {
      ...
      expect(event.isInitiativeApply, isNotNull,
          reason: 'JMReceiveApplyJoinGroupApprovalEvent.isInitiativeApply is null');
      ...
      expect(event.joinGroupUsers, isNotNull,
          reason: 'JMReceiveApplyJoinGroupApprovalEvent.joinGroupUsers is null');
...
// addReceiveGroupAdminApprovalListener
  void testSendMessageAPIs() async {
    ...
    test('sendCustomMessage', () async {
      JMCustomMessage msg = await jmessage
          .sendCustomMessage(type: kMockGroup, customObject: {'customKey1': 'customValue1'});
      verifyMessage(msg);
    });
    ...
    test('retractMessage', () async {
      ...
      await jmessage.retractMessage(target: kMockUser, serverMessageId: msg.serverMessageId);
    });
  }
    ...
    group('$JmessageFlutter', () {
      ...
      test('updateMyInfo', () async {
        ...
        await jmessage.updateMyInfo(
            birthday: DateTime.now().millisecondsSinceEpoch, gender: _gender, extras: _extras);
      ...
      test('createGroup', () async {
        String gid = await jmessage.createGroup(
            groupType: JMGroupType.private, name: kMockGroupName, desc: kMockGroupDesc);
      ...
      test('createConversation', () async {
        ...
        JMConversationInfo singleConversation = await jmessage.createConversation(
            target: kMockUser); //@required dynamic target, //(JMSingle | JMGroup | JMChatRoom)
      ...
      test('setConversationExtras', () async {
        ...
        JMConversationInfo conversationInfo = await jmessage
            .setConversationExtras(type: kMockUser, extras: {'extrasKey1': 'extrasValue'});
        verifyConversation(conversationInfo);
        print('test    setConversationExtras done');
      });

      test('getHistoryMessages', () async {
        List msgs = await jmessage.getHistoryMessages(type: kMockUser, from: 0, limit: 20);
      ...
      test('getMessageById', () async {
        ...
        var message = await jmessage.getMessageById(type: kMockUser, messageId: msg.id);
      ...
      test('updateFriendNoteName', () async {
        await jmessage.updateFriendNoteName(
            username: kMockTargetUserName, noteName: 'test   update FriendNoteName');
      ...
      test('updateFriendNoteText', () async {
        await jmessage.updateFriendNoteText(
            username: kMockTargetUserName, noteText: 'test   update FriendNoteText');
      ...
      test('updateGroupInfo', () async {
        ...
        await jmessage.updateGroupInfo(id: kMockGroupId, newName: kMockName, newDesc: kMockDesc);
      ...
      test('setGroupMemberSilence', () async {
        ...
        bool isSilenceMember =
            await jmessage.isSilenceMember(groupId: kMockGroupId, username: '0002');
        ...
        isSilenceMember = await jmessage.isSilenceMember(groupId: kMockGroupId, username: '0002');
      ...
      test('setGroupNickname', () async {
        ...
        await jmessage.setGroupNickname(
            groupId: kMockGroupId, username: '0002', nickName: kMockgroupNickName);
      ...
      test('changeGroupType', () async {
        await jmessage.changeGroupType(groupId: kMockGroupId, type: JMGroupType.public);

        await jmessage.changeGroupType(groupId: kMockGroupId, type: JMGroupType.private);
      });
        ...
        child: Column(
          children: <Widget>[
            Container(
              ...
              height: 35,
              color: Colors.brown,
              child: CustomTextField(hintText: "请输入登录的 username", controller: usernameTextEC1),
            ),
            Row(
            ...
            Container(
              ...
              height: 35,
              color: Colors.brown,
              child: CustomTextField(hintText: "请输入username", controller: usernameTextEC2),
            ),
            Row(
              mainAxisAlignment: MainAxisAlignment.center,
              children: <Widget>[
                ...
                Text(" "),
                CustomButton(title: "发送位置消息", onPressed: demoSendLocationMessage),
              ],
            ),
            Row(
              mainAxisAlignment: MainAxisAlignment.center,
                      ...
                      jmessage.getMyInfo().then((JMUserInfo? userInfo) {
                        if (userInfo != null) {
                          Navigator.push(context,
                              MaterialPageRoute(builder: (context) => ConversationManageView()));
                        } else {
                          setState(() {
                            _result = " 请先登录 ";
                          });
                ...
                CustomButton(
                  ...
                  onPressed: () {
                    jmessage.getMyInfo().then((JMUserInfo? userInfo) {
                      if (userInfo != null) {
                        Navigator.push(
                            context, MaterialPageRoute(builder: (context) => GroupManageView()));
                      } else {
                        setState(() {
                          _result = " 请先登录 ";
                        });
