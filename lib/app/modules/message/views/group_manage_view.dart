...
import 'package:inspector/app/modules/message/views/temp.dart';
...
class _GroupManageViewState extends State<GroupManageView> {
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(
        title: Text("群组列表", style: TextStyle(fontSize: 20)),
        leading: IconButton(
            icon: Image.asset("assets/nav_close.png"),
            onPressed: () {
  ...
  Widget _buildContentView() {
    return Column(
      children: <Widget>[
            ...
            children: <Widget>[
              ...
              Expanded(
                child: Column(
                  ...
                  children: <Widget>[
                    CustomButton(title: "私有群列表", onPressed: demoGetGidList),
                    CustomButton(title: "公开群列表", onPressed: demoGetPublicGroupInfos),
                  ],
                ),
                flex: 1,
              ),
        ...
        CustomTextField(hintText: "请输入username/group name", controller: usernameTextEC1),
        Row(
          children: <Widget>[
            Text(" "),
            CustomButton(title: "成员列表", onPressed: demoGetMemberlist),
            Text(" "),
            CustomButton(title: "加入成员", onPressed: demoAddMember),
            Text(" "),
            CustomButton(title: "移除成员", onPressed: demoRemoveMember),
            Text(" "),
            CustomButton(title: "申请加入", onPressed: demoApplyJoinGroup),
          ],
        ),
        Row(
          children: <Widget>[
            ...
            Text(" "),
            CustomButton(title: "创建群聊会话", onPressed: demoCreatConversation),
          ],
        ),
        Expanded(
          child: Container(
  ...
  void demoCreateGroup(JMGroupType type) async {
    ...
    String groupIdString =
        await jmessage.createGroup(groupType: type, name: name, desc: "$name-的群描述信息");
    setState(() {
      _loading = false;
